...
def test_linear_interpolation_fills_bounded_gap():
    ...
    load = pd.DataFrame(
        {"ALB": [10.0, float("nan"), 14.0]},
        index=index,
    )
    durations = pd.DataFrame(
        {"ALB": [pd.Timedelta(0), pd.Timedelta(hours=1), pd.Timedelta(0)]},
        index=index,
    )
...
def test_linear_interpolation_respects_max_gap():
    ...
    load = pd.DataFrame(
        {"ALB": [10.0, float("nan"), float("nan"), 16.0]},
        index=index,
    )
    durations = pd.DataFrame(
        {
...
def test_copy_periods_fills_from_previous_period():
    ...
    load = pd.DataFrame(
        {"ALB": [10.0, 11.0, float("nan"), 13.0, 14.0]},
        index=index,
    )
    durations = pd.DataFrame(
        pd.Timedelta(0),
...
def test_average_periods_uses_complete_sources():
    ...
    load = pd.DataFrame(
        {"ALB": [10.0, 12.0, float("nan"), 16.0, 18.0]},
        index=index,
    )
    durations = pd.DataFrame(
        pd.Timedelta(0),
