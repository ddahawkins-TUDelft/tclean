...
def test_validate_defaults_peer_aggregation_mode_to_median():
    ...
    result = validate_quality_test(
        {
            ...
            "threshold": {"value_mode": "fixed", "value": 10},
        },
        grid=_grid(),
    )
    ...
    assert result["threshold"] == {"value_mode": "fixed", "value": 10}
...
def test_validate_accepts_relative_mean_mode():
    ...
    assert result["threshold"] == {"value_mode": "fixed", "value": 0.1}
...
@pytest.mark.parametrize(
    "test",
    [
        {"name": "range", "method": "range", "minimum": 0},
        {
            "name": "value_run",
            "method": "value_run",
            "value": 0,
...
def test_validate_quality_tests_requires_explicit_value_specs(test):
    """Fail fast when a data-scale scalar is supplied without value_mode."""
    with pytest.raises(ValueError, match="value specification"):
        validate_quality_tests([test], grid=_grid())


def test_validate_quality_tests_normalizes_nested_derived_value_spec():
    """Normalize nested value specifications through the public validator."""
    result = validate_quality_tests(
        [
            {
    ...
    assert result[0]["maximum"] == {"value_mode": "median", "multiplier": 1.0}


def test_validate_quality_tests_rejects_invalid_nested_value_spec():
    """Reject malformed nested value specifications through the public API."""
...
def test_validate_range_accepts_prior_failure_controls_for_derived_bound():
    """Allow prior-failure controls when a range bound is derived."""
    result = validate_quality_test(
        {
            "name": "derived_maximum",
            "method": "range",
...
def test_validate_rate_of_change_rejects_derived_relative_threshold():
    """Keep relative rate-of-change thresholds dimensionless and fixed."""
        ...
        validate_quality_test(
            {
                ...
                "method": "rate_of_change",
                "difference_mode": "relative",
                "threshold": {"value_mode": "median", "multiplier": 0.1},
            },
            grid=_grid(),
        )


def test_validate_rate_of_change_accepts_derived_reference_magnitude_threshold():
    """Allow the relative reference-magnitude floor to derive from focal data."""
    result = validate_quality_test(
        {
            "name": "relative_change",
            "method": "rate_of_change",
            ...
            "reference_magnitude_threshold": {"value_mode": "median"},
        },
        grid=_grid(),
    )
...
def test_validate_source_disagreement_rejects_derived_relative_threshold():
    """Keep relative source-disagreement thresholds dimensionless and fixed."""
    with pytest.raises(ValueError, match="dimensionless"):
        validate_quality_test(
            {
                "name": "relative_disagreement",
                "method": "source_disagreement",
                "difference_mode": "relative",
                "threshold": {"value_mode": "median", "multiplier": 0.1},
            },
