...
def _mask(values: list[bool]) -> pd.Series:
    ...
    return pd.Series(values, index=pd.DatetimeIndex(grid.target_index), dtype=bool)
...
def test_failure_mask_to_periods_returns_empty_for_no_failures():
    ...
    result = failure_mask_to_periods(
        _mask([False, False, False, False, False, False]), grid=_grid()
    )
...
def test_failure_mask_to_periods_reports_single_failed_value():
    ...
    result = failure_mask_to_periods(
        _mask([False, False, True, False, False, False]), grid=_grid()
    )
...
def test_failure_mask_to_periods_merges_adjacent_failures():
    ...
    result = failure_mask_to_periods(
        _mask([False, True, True, True, False, False]), grid=_grid()
    )

    assert result == [
        (pd.Timestamp("2026-01-01T01:00:00Z"), pd.Timestamp("2026-01-01T04:00:00Z"))
    ]
...
def test_failure_mask_to_periods_keeps_separate_runs():
    ...
    result = failure_mask_to_periods(
        _mask([True, True, False, True, False, True]), grid=_grid()
    )

    assert result == [
        (pd.Timestamp("2026-01-01T00:00:00Z"), pd.Timestamp("2026-01-01T02:00:00Z")),
        (pd.Timestamp("2026-01-01T03:00:00Z"), pd.Timestamp("2026-01-01T04:00:00Z")),
        (pd.Timestamp("2026-01-01T05:00:00Z"), pd.Timestamp("2026-01-01T06:00:00Z")),
    ]
...
def test_failure_mask_to_periods_uses_grid_end_for_final_run():
    ...
    result = failure_mask_to_periods(
        _mask([False, False, False, True, True, True]), grid=_grid()
    )

    assert result == [
        (pd.Timestamp("2026-01-01T03:00:00Z"), pd.Timestamp("2026-01-01T06:00:00Z"))
    ]
...
def test_failure_mask_to_periods_accepts_nullable_boolean_dtype():
    ...
    assert result == [
        (pd.Timestamp("2026-01-01T01:00:00Z"), pd.Timestamp("2026-01-01T03:00:00Z"))
    ]
...
def test_failure_mask_to_periods_rejects_non_series():
    ...
    with pytest.raises(TypeError, match="must be a pandas Series"):
        failure_mask_to_periods([False, True, False], grid=_grid())
...
def test_failure_mask_to_periods_rejects_non_boolean_values():
    ...
    mask = pd.Series(
        [0, 1, 1, 0, 0, 0], index=pd.DatetimeIndex(grid.target_index), dtype=int
    )

    with pytest.raises(TypeError, match="must contain boolean values"):
...
def test_failure_mask_to_periods_rejects_wrong_index():
    ...
    mask = pd.Series(
        [False, True, True, False, False, False],
        index=pd.date_range("2026-01-01T01:00:00Z", periods=6, freq="1h"),
        dtype=bool,
    )

    with pytest.raises(ValueError, match="must exactly match the target time grid"):
