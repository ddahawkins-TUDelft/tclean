...
def _grid() -> TimeGrid:
    ...
    return TimeGrid(
        start="2026-01-01T00:00:00Z", end="2026-01-01T12:00:00Z", frequency="1h"
    )
...
def _test(*, minimum_matches: int = 3, tolerance: float = 0.0) -> dict:
...
def test_evaluate_applies_independently_to_contexts():
    ...
    data = pd.DataFrame(
        {
            "A": [1, 2, 9, 8, 1, 2, 7, 6, 1, 2, 5, 4],
            "B": [9, 9, 3, 4, 8, 8, 3, 4, 7, 7, 3, 4],
        },
...
def test_build_details_points_each_block_to_its_matches():
    ...
    assert block["start"] == pd.Timestamp("2026-01-01T00:00:00Z")
    assert block["end"] == pd.Timestamp("2026-01-01T02:00:00Z")
    assert block["match_count"] == 3

    assert [match["start"] for match in block["matches"]] == [
        pd.Timestamp("2026-01-01T04:00:00Z"),
        pd.Timestamp("2026-01-01T08:00:00Z"),
    ]

    assert all(
        match["maximum_absolute_difference"] == pytest.approx(0.0)
...
def test_build_details_preserves_adjacent_constituent_blocks():
    ...
    assert [block["start"] for block in details["matched_blocks"]] == [
