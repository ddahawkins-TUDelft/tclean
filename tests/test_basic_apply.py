...
from tclean.basic.apply import (
...
def test_fill_basic_gaps_marks_unresolved_values_missing():
    ...
    assert (
        provenance.loc[index[1:3], "ALB"] == "missing"
    ).all()
...
def test_fill_basic_gaps_rejects_unknown_method():
    ...
    with pytest.raises(
        ValueError,
        match="Unsupported gap-filling method",
    ):
        fill_basic_gaps(
            load,
            cleaning_method=cleaning_method,
