...
from tclean.data_quality.methods.level_shift import (
    build_details,
    evaluate,
)
...
def test_evaluate_applies_independently_to_contexts():
    ...
    assert result.index[result["B"]].tolist() == [
...
def test_build_details_uses_full_support_region():
    ...
    assert details["change_point"] == pd.Timestamp(
    ...
    assert details["pre_evidence_start"] == pd.Timestamp(
        "2026-01-01T02:00:00Z"
    )
    assert details["post_evidence_end"] == pd.Timestamp(
