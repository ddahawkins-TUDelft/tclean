...
def test_infer_regular_timestep_rejects_non_datetime_index():
    ...
    with pytest.raises(
        TypeError,
        match="Load data must use a pandas DatetimeIndex.",
    ):
...
def test_infer_regular_timestep_rejects_unsorted_index():
    ...
    with pytest.raises(
        ValueError,
        match="Load timestamps must be sorted",
    ):
        infer_regular_timestep(index)
...
def test_infer_regular_timestep_rejects_irregular_index():
    ...
    with pytest.raises(
        ValueError,
        match="complete, regular time index",
    ):
        infer_regular_timestep(index)
...
def test_validate_load_accepts_hourly_numeric_dataframe():
    ...
    load = pd.DataFrame(
        {
            "ALB": [100.0, 101.0, 102.0],
            "GBR": [200.0, 201.0, 202.0],
        },
        index=index,
    )
...
def test_validate_load_rejects_non_hourly_data():
    ...
    with pytest.raises(
        ValueError,
        match="currently expects hourly load data",
    ):
        validate_load(load)
...
def test_validate_load_rejects_non_numeric_columns():
    ...
    with pytest.raises(
        TypeError,
        match="All load columns must be numeric.",
    ):
        validate_load(load)
