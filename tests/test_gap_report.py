...
def test_build_gap_report_returns_empty_report_when_disabled():
    ...
    load = pd.DataFrame({"ALB": [10.0, float("nan"), 12.0]}, index=index)

    result = build_gap_report(load, enabled=False)
...
def test_build_gap_report_marks_start_boundary_gap():
    ...
    load = pd.DataFrame({"ALB": [float("nan"), float("nan"), 12.0, 13.0]}, index=index)
...
def test_build_gap_report_marks_end_boundary_gap():
    ...
    index = pd.date_range("2026-01-01 00:00", periods=4, freq="h", tz="UTC")
    load = pd.DataFrame({"ALB": [10.0, 11.0, float("nan"), float("nan")]}, index=index)
...
def test_build_gap_report_separates_multiple_gaps():
    ...
    index = pd.date_range("2026-01-01 00:00", periods=6, freq="h", tz="UTC")
    load = pd.DataFrame(
