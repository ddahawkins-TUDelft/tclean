...
def test_read_external_profile_reads_valid_csv(tmp_path: Path):
    ...
    result = read_external_profile(path, grid=_grid())
...
def test_read_external_profile_converts_offsets_to_utc(tmp_path: Path):
...
def test_read_external_profile_rejects_missing_column(tmp_path: Path):
    """Reject external profiles missing a required column."""
    path = tmp_path / "profile.csv"
    path.write_text("timestamp\n2026-01-01T00:00:00Z\n")

    with pytest.raises(pandera.errors.SchemaErrors):
        read_external_profile(path, grid=_grid())
...
def test_read_external_profile_rejects_incompatible_frequency(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="integer multiples"):
        read_external_profile(path, grid=_grid("30min"))
