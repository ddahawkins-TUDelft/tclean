...
from tclean.advanced.methods.external_profile import load_external_profile


def test_load_external_profile_reads_valid_csv(tmp_path: Path):
    """Load a valid external profile as a sorted UTC demand series."""
    path = tmp_path / "profile.csv"
    path.write_text(
        "timestamp,demand\n2026-01-01T01:00:00Z,110\n2026-01-01T00:00:00Z,100\n"
    )

    result = load_external_profile(path)
...
def test_load_external_profile_converts_offsets_to_utc(tmp_path: Path):
...
def test_load_external_profile_rejects_missing_column(tmp_path: Path):
    """Reject external profiles missing a required column."""
    path = tmp_path / "profile.csv"
    path.write_text("timestamp\n2026-01-01T00:00:00Z\n")

    with pytest.raises(pandera.errors.SchemaErrors):
        load_external_profile(path)


def test_load_external_profile_rejects_extra_column(tmp_path: Path):
...
def test_load_external_profile_rejects_missing_demand(tmp_path: Path):
...
def test_load_external_profile_rejects_non_numeric_demand(tmp_path: Path):
...
def test_load_external_profile_rejects_duplicate_timestamps(tmp_path: Path):
...
def test_load_external_profile_rejects_subhourly_timestamp(tmp_path: Path):
...
def test_load_external_profile_rejects_invalid_timestamp(tmp_path: Path):
...
def test_load_external_profile_rejects_reversed_columns(tmp_path: Path):
...
def test_load_external_profile_rejects_whitespace_in_column_name(tmp_path: Path):
