"""Pandera schemas for gap-filling data contracts."""
...
ADVANCED_FILL_RULES_SCHEMA = pa.DataFrameSchema(
    ...
    checks=[
        ...
        pa.Check(
            ...
            error=(
                "Advanced rules using external_profile or construct_from_sources "
                "must define a source, while leave_missing rules must not define one."
            ),
        ),
    ],
    strict=True,
...
SOURCE_CAPABILITIES_SCHEMA = pa.DataFrameSchema(
    ...
    checks=[
        ...
        pa.Check(
            ...
            error=(
                "A source must use either wildcard context coverage or explicit "
                "context coverage, not both."
            ),
        ),
    ],
    strict=True,
