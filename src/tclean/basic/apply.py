...
from tclean.basic.methods.average_periods import (
    METHOD_NAME as AVERAGE_PERIODS,
)
from tclean.basic.methods.average_periods import apply_average_periods
from tclean.basic.methods.copy_periods import (
    METHOD_NAME as COPY_PERIODS,
)
...
def _validate_cleaning_method(
    *,
    ...
    if not cleaning_method.index.equals(load.index):
        raise ValueError(
            "Cleaning-method provenance must use the same index "
            "as the load data."
        )

    if not cleaning_method.columns.equals(load.columns):
        raise ValueError(
            "Cleaning-method provenance must use the same columns "
            "as the load data."
        )
